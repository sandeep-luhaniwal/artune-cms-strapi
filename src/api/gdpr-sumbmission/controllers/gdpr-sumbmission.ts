/**
 * gdpr-sumbmission controller
 */

import { factories } from '@strapi/strapi';
import ExcelJS from 'exceljs';

export default factories.createCoreController('api::gdpr-sumbmission.gdpr-sumbmission', ({ strapi }) => ({
  async exportSubmissions(ctx) {
    try {
      const submissions = await strapi.documents('api::gdpr-sumbmission.gdpr-sumbmission').findMany({
        limit: 10000,
      });
      ...
      ctx.set('Content-Disposition', 'attachment; filename=gdpr_submissions.xlsx');
      ctx.body = buffer;
    } catch (error: any) {
      strapi.log.error('Error exporting GDPR submissions:', error);
  ...
  async create(ctx) {
    try {
      ...
      const apiUid = 'api::gdpr-sumbmission.gdpr-sumbmission';
      const entryData = {
