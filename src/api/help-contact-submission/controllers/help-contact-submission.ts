/**
 * help-contact-submission controller
 */

import { factories } from '@strapi/strapi';
import ExcelJS from 'exceljs';

export default factories.createCoreController('api::help-contact-submission.help-contact-submission', ({ strapi }) => ({
  async exportSubmissions(ctx) {
    try {
      const submissions = await strapi.documents('api::help-contact-submission.help-contact-submission').findMany({
        limit: 10000,
      });
      ...
      ctx.set('Content-Disposition', 'attachment; filename=help_contact_submissions.xlsx');
      ctx.body = buffer;
    } catch (error: any) {
      strapi.log.error('Error exporting help contact submissions:', error);
  ...
  async create(ctx) {
    try {
      ...
      const apiUid = 'api::help-contact-submission.help-contact-submission';
      const entryData = {
