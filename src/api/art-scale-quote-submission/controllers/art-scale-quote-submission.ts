/**
 * art-scale-quote-submission controller
 */

import { factories } from '@strapi/strapi';
import ExcelJS from 'exceljs';

export default factories.createCoreController('api::art-scale-quote-submission.art-scale-quote-submission', ({ strapi }) => ({
  async exportSubmissions(ctx) {
    try {
      const submissions = await strapi.documents('api::art-scale-quote-submission.art-scale-quote-submission').findMany({
        limit: 10000,
      });
      ...
      ctx.set('Content-Disposition', 'attachment; filename=art_scale_quote_submissions.xlsx');
      ctx.body = buffer;
    } catch (error: any) {
      strapi.log.error('Error exporting art scale quote submissions:', error);
  ...
  async create(ctx) {
    try {
      ...
      const apiUid = 'api::art-scale-quote-submission.art-scale-quote-submission';
      const entryData = {
